...
const s = storiesOf("Net prototype|Colors", module);
s.addDecorator(withKnobs);

s.add("Dark theme", () => {
  const voice = select("Voice", ["beeb", "user"], "beeb");
  const brand = select("Brand", [null, ...Object.keys(colors.brands)], null);
  return (
    <Net
...
const sb = storiesOf("Net prototype|Colors/With BBC brands", module);
