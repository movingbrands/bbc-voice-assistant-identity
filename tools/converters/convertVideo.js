...
const convertVideo = ({
  ...
  new Promise((resolve, reject) => {
    ...
    if (!targetCodec)
      reject(
        `Invalid format: ${format}. Please choose one of ${Object.keys(
          codecs
        ).join(", ")}`
      );
...
const generateScreenshot = ({ input, output, format = "jpg" }) =>
  new Promise((resolve, reject) => {
    const targetFile = `${output}.${format}`;

    const screenshot = ffmpeg(input).seek("0.0");

    screenshot.save(targetFile);
    resolve(true);
  });

const convertVideoForWeb = ({ input, output }) =>
  new Promise((resolve, reject) => {
    Promise.all([convertVideo({ input, output, format: "mp4" })])
      .then(r => {
        const result = {
          sources: [
            {
